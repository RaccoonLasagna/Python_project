...
class Quotation_Edit(Frame):
    def __init__(self, parent, controller):
        ...
        combobox_category = ttk.Combobox(self,values=self.category_values ,state="readonly", textvariable=self.category)
        combobox_category.current(0)
        combobox_category.bind("<<ComboboxSelected>>", self.load_items)

        self.item = StringVar()
        self.combobox_items = ttk.Combobox(self,values=["รอการเลือกหมวดหมู่"], textvariable=self.item, state='readonly')

        # listbox to store added item
        self.added_items = Listbox(self)

        # buttons
        add_button = Button(self, text="เพื่ม", command=self.add_item)
        create_button = Button(self, text="เขียนใบเสนอราคา", command=self.create_quotation)
        back_button = Button(self, text="ย้อนกลับ", command=lambda: controller.change_frame("mm"))
        
        # grid
        Label(self, text="ที่อยู่ลูกค้า").pack()
        self.address_entry.pack(fill=X)
        Label(self, text="เลือกรายการ").pack()
        combobox_category.pack(fill=BOTH)
        self.combobox_items.pack(fill=X)
        add_button.pack(fill=X)
        self.added_items.pack(fill=BOTH)
    ...
    def create_quotation(self):
        ...
        shutil.copyfile(r"util\base.xlsx", save_location)
...
if __name__ == "__main__":
    test = Tk()
    config = Quotation_Edit(test, test)
    config.pack()
    test.mainloop()